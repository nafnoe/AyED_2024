...
if not os.path.exists(archFiscAdmin):
    archLoAdmi= open(archFiscAdmin, "w+b") # genera el archivo binario si no existe
else:
    archLoAdmi= open(archFiscAdmin, "r+b") # lee y escribe el archivo binario
...
def fn_busquedaSecAdm(email):
    return fn_BusquedaSec(email,archFiscAdmin,archLoAdmi)
...
def pr_iniciarSesion():
    ...
    while intento < 3 and not (autenticado):
        ...
        fn_BusquedaSec(email,archFiscEst,archLoEst)
        """
        indice = fn_BusquedaSec(email)
        if (
            indice != -1
            and Estudiante.contrasena == contrasena
        ...
        elif indice == fn_BusquedaSec(Moderador):
            if (
                indice != -1
                and Moderador.contrasena== contrasena
                and Moderador.estado== "activo"
            ):
                autenticado = True
                m = Moderador.estado
        else:
            # TERCER CASO BUSCAMOS POR ADMINISTRADOR EXISTENTE
            indice = fn_BusquedaSec(Administrador)
            if (
                indice != -1
                and Administrador.contrasena== contrasena
...
def pr_logueo():
    ...
    cant_adm = cant_registro(archLoAdmi,archFiscAdmin)
