...
archFiscEst = os.path.join(".", "estudiantes.dat")
...
archFiscMode = os.path.join(".", "moderadores.dat")
...
archFiscAdmin = os.path.join(".", "administrador.dat")
...
archFiscLike = os.path.join(".", "like.dat")
...
archFiscRepo = os.path.join(".", "reportes.dat")
...
def crear_administrador():
    admi_datos = [["administrador@ayed.com", "888111"]]
    if os.path.getsize(archFiscAdmin) == 0:
        admi = Administrador()
        for i in range(1):
            admi.id_usuario = i
            admi.email = admi_datos[i][0]
            admi.contrasena = admi_datos[i][1]
...
def crear_moderador():
    ...
    mode_datos = [["moderador@ayed.com", "999111"]]

    if os.path.getsize(archFiscMode) == 0:
        mode = Moderador()
        for i in range(1):
            mode.id_usuario = i
            mode.email = mode_datos[i][0]
            mode.contrasena = mode_datos[i][1]
            mode.estado = True
...
def crear_estudiante():
    ...
    if os.path.getsize(archFiscEst) == 0:
        ...
        for i in range(4):
            est.id_usuario = i
            est.email = est_datos[i][0]
            est.nombreyapellido = est_datos[i][1]
            est.sexo = est_datos[i][2]
            est.contrasena = est_datos[i][3]
            est.hobbies = est_datos[i][4]
            est.materia_favorita = est_datos[i][5]
            est.deporte_favorito = est_datos[i][6]
            est.materia_fuerte = est_datos[i][7]
            est.materia_debil = est_datos[i][8]
            est.biografia = est_datos[i][9]
            est.pais = est_datos[i][10]
            est.ciudad = est_datos[i][11]
            est.fecha_nacimiento = est_datos[i][12]
            est.estado = True
...
def fn_busquedaRegistroModerador(email):
    global archFiscMode, archLoMode
...
def pr_iniciarSesion():
    ...
    while intento < 3 and not (autenticado):
        ...
        if RegEst.email.strip() == email and RegEst.contrasena.strip() == contrasena:
            autenticado = True
            usuarioData = RegEst
            pr_menu_est()

        if RegMod.email.strip() == email and RegMod.contrasena.strip() == contrasena:
            autenticado = True
            usuarioData = RegMod
            pr_menu_mod()

        if RegMod.email.strip() == email and RegAdm.contrasena.strip() == contrasena:
            autenticado = True
            usuarioData = RegAdm
            pr_menu_adm()
